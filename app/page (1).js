...
export default function SecretCardSociety() {
  ...
  const normalizeType = (t) => {
    const validTypes = ['Single Card', 'Slab', 'Booster Box', 'Booster Pack', 'ETB', 'Collection Box', 'Tin', 'Bundle', 'Other']
    if (validTypes.includes(t)) return t
    const tl = t.toLowerCase()
    if (tl.includes('slab')) return 'Slab'
    if (tl.includes('booster') && tl.includes('box')) return 'Booster Box'
    if (tl.includes('booster') || tl.includes('pack')) return 'Booster Pack'
    if (tl.includes('etb') || tl.includes('elite')) return 'ETB'
    if (tl.includes('collection')) return 'Collection Box'
    if (tl.includes('tin')) return 'Tin'
    if (tl.includes('bundle')) return 'Bundle'
    return 'Single Card'
  }

  const normalizeLang = (l) => {
    const validLangs = ['English', 'Japanese', 'Korean', 'Chinese (Traditional)', 'Chinese (Simplified)']
    if (validLangs.includes(l)) return l
    const ll = l.toLowerCase()
    if (ll.includes('jap') || ll === 'jp' || ll === 'jpn') return 'Japanese'
    if (ll.includes('kor') || ll === 'kr') return 'Korean'
    if (ll.includes('chinese') && ll.includes('trad')) return 'Chinese (Traditional)'
    if (ll.includes('chinese') || ll === 'cn' || ll === 'chs') return 'Chinese (Simplified)'
    return 'English'
  }
  ...
  const normalizeCond = (c) => {
    ...
    if (validConds.includes(cu)) return cu
    const cl = c.toLowerCase()
    if (cl.includes('near') || cl === 'mint') return 'NM'
    if (cl.includes('light')) return 'LP'
    if (cl.includes('moderate')) return 'MP'
    if (cl.includes('heavy')) return 'HP'
    if (cl.includes('damage')) return 'DMG'
    return 'NM'
  }
  ...
  const handleImport = async () => {
    ...
    const validRows = importData.filter(row => {
      const name = importMapping.name !== undefined ? String(row[importMapping.name] || '') : ''
      const set = importMapping.set !== undefined ? String(row[importMapping.set] || '') : ''
      const price = importMapping.purchasePrice !== undefined ? parseFloat(row[importMapping.purchasePrice]) : 0
      const value = importMapping.currentValue !== undefined ? parseFloat(row[importMapping.currentValue]) : 0
      return name && set && !isNaN(price) && !isNaN(value)
    })
    ...
    for (let i = 0; i < validRows.length; i++) {
      ...
      try {
        ...
        const set = String(row[importMapping.set] || '').trim()
        ...
        const grade = importMapping.grade !== undefined ? String(row[importMapping.grade] || '').trim() : ''
        const quantity = importMapping.quantity !== undefined ? parseInt(row[importMapping.quantity]) || 1 : 1
        const purchasePrice = parseFloat(row[importMapping.purchasePrice]) || 0
        const currentValue = parseFloat(row[importMapping.currentValue]) || 0
        ...
        if (existing) {
          ...
          const avgCost = ((existing.purchasePrice * existing.quantity) + (purchasePrice * quantity)) / totalQty
          await supabase.from('products').update({ quantity: totalQty, purchase_price: Math.round(avgCost * 100) / 100, current_value: currentValue, updated_at: new Date().toISOString() }).eq('id', existing.id)
        } else {
          await supabase.from('products').insert({ name, set_name: set, product_type: productType, language, condition, grade, quantity, purchase_price: purchasePrice, current_value: currentValue })
        }
        imported++
    ...
    await loadProducts()
    setSyncing(false)
    setImportError(errors > 0 ? `Imported ${imported} items with ${errors} errors` : `Successfully imported ${imported} items!`)
  ...
  const downloadTemplate = () => {
    const template = 'Name,Set,Type,Language,Condition,Grade,Quantity,Purchase Price,Current Value\nCharizard VMAX,Champions Path,Single Card,English,NM,,2,150.00,200.00\nPikachu V,Vivid Voltage,Single Card,Japanese,NM,PSA 10,1,50.00,75.00\nBooster Box,Scarlet & Violet,Booster Box,English,NM,,1,140.00,160.00'
    const blob = new Blob([template], { type: 'text/csv' }), url = URL.createObjectURL(blob), a = document.createElement('a')
    a.href = url; a.download = 'SCS_Import_Template.csv'; a.click(); URL.revokeObjectURL(url)
  }
  ...
  const salesByMonth = sales.reduce((acc, sale) => { const month = new Date(sale.date).toLocaleDateString('en-US', { month: 'short', year: '2-digit' }); acc[month] = (acc[month] || 0) + sale.profitLoss; return acc }, {})
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#111827', padding: '12px', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        {showImportModal && (
          <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', zIndex: 50 }}>
            <div style={{ backgroundColor: '#1f2937', borderRadius: '8px', padding: '16px', width: '100%', maxWidth: '600px', maxHeight: '90vh', overflow: 'auto', border: '1px solid #374151' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
                {['Upload', 'Map Columns', 'Preview', 'Import'].map((step, i) => (
                  <div key={step} style={{ flex: 1, textAlign: 'center' }}>
                    <div style={{ width: '24px', height: '24px', borderRadius: '50%', backgroundColor: importStep > i + 1 ? '#16a34a' : importStep === i + 1 ? '#2563eb' : '#374151', color: 'white', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', marginBottom: '4px' }}>{importStep > i + 1 ? <Icons.Check /> : i + 1}</div>
                    <div style={{ fontSize: '10px', color: importStep === i + 1 ? '#60a5fa' : '#6b7280' }}>{step}</div>
                  </div>
                ))}
              </div>
              ...
              {importStep === 1 && (
                <div>
                  <div style={{ border: '2px dashed #374151', borderRadius: '8px', padding: '32px', textAlign: 'center', cursor: 'pointer', marginBottom: '16px', backgroundColor: '#111827' }} onClick={() => fileInputRef.current?.click()}>
                    <Icons.Upload /><p style={{ color: '#9ca3af', margin: '8px 0 4px' }}>Click to upload Excel file</p><p style={{ color: '#6b7280', fontSize: '12px', margin: 0 }}>Supports .xlsx, .xls, .csv</p>
                    <input ref={fileInputRef} type="file" accept=".xlsx,.xls,.csv" onChange={handleFileUpload} style={{ display: 'none' }} />
                  </div>
                  <button onClick={downloadTemplate} style={{ width: '100%', padding: '10px', backgroundColor: '#374151', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}><Icons.Download /> Download Template</button>
                  <div style={{ marginTop: '16px', padding: '12px', backgroundColor: '#111827', borderRadius: '8px', fontSize: '12px', color: '#9ca3af' }}>
                    <p style={{ margin: '0 0 8px', fontWeight: '500', color: '#d1d5db' }}>Required columns:</p>
                    <ul style={{ margin: 0, paddingLeft: '16px' }}><li>Name (Product/Card name)</li><li>Set (Expansion/Set name)</li><li>Purchase Price (Cost)</li><li>Current Value</li></ul>
                    <p style={{ margin: '8px 0 0', fontWeight: '500', color: '#d1d5db' }}>Optional:</p>
                    <ul style={{ margin: 0, paddingLeft: '16px' }}><li>Type, Language, Condition, Grade, Quantity</li></ul>
                  </div>
                </div>
              )}
              ...
              {importStep === 2 && (
                <div>
                  <p style={{ color: '#9ca3af', fontSize: '13px', marginBottom: '16px' }}>Map your Excel columns to inventory fields. Found {importData.length} rows.</p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {requiredFields.map(field => (
                      <div key={field.key} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <div style={{ width: '120px', fontSize: '13px', color: field.required ? '#fb923c' : '#9ca3af' }}>{field.label}{field.required && ' *'}</div>
                        <select value={importMapping[field.key] ?? ''} onChange={(e) => setImportMapping({ ...importMapping, [field.key]: e.target.value === '' ? undefined : parseInt(e.target.value) })} style={{ flex: 1, padding: '8px', backgroundColor: '#374151', border: '1px solid #4b5563', borderRadius: '6px', color: 'white', fontSize: '13px' }}>
                          <option value="">-- Select column --</option>
                          {excelHeaders.map((header, i) => <option key={i} value={i}>{header || `Column ${i + 1}`}</option>)}
                        </select>
                      </div>
                    ))}
                  </div>
                  <div style={{ display: 'flex', gap: '8px', marginTop: '16px' }}>
                    <button onClick={() => setImportStep(1)} style={{ flex: 1, padding: '10px', backgroundColor: '#374151', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px' }}>Back</button>
                    <button onClick={() => { const missing = validateMapping(); if (missing.length > 0) setImportError(`Please map required fields: ${missing.join(', ')}`); else { setImportError(''); generatePreview() } }} style={{ flex: 1, padding: '10px', backgroundColor: '#2563eb', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px' }}>Preview</button>
                  </div>
                </div>
              )}
              ...
              {importStep === 3 && (
                <div>
                  <p style={{ color: '#9ca3af', fontSize: '13px', marginBottom: '16px' }}>Preview (showing first {importPreview.length} of {importData.length} rows)</p>
                  <div style={{ overflowX: 'auto', marginBottom: '16px' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
                      <thead><tr style={{ backgroundColor: '#111827' }}><th style={{ padding: '8px', textAlign: 'left', color: '#9ca3af', borderBottom: '1px solid #374151' }}>Name</th><th style={{ padding: '8px', textAlign: 'left', color: '#9ca3af', borderBottom: '1px solid #374151' }}>Set</th><th style={{ padding: '8px', textAlign: 'left', color: '#9ca3af', borderBottom: '1px solid #374151' }}>Type</th><th style={{ padding: '8px', textAlign: 'left', color: '#9ca3af', borderBottom: '1px solid #374151' }}>Qty</th><th style={{ padding: '8px', textAlign: 'left', color: '#9ca3af', borderBottom: '1px solid #374151' }}>Cost</th><th style={{ padding: '8px', textAlign: 'left', color: '#9ca3af', borderBottom: '1px solid #374151' }}>Value</th></tr></thead>
                      <tbody>{importPreview.map((item, i) => <tr key={i}><td style={{ padding: '8px', color: '#fb923c', borderBottom: '1px solid #374151' }}>{item.name}</td><td style={{ padding: '8px', color: '#d1d5db', borderBottom: '1px solid #374151' }}>{item.set}</td><td style={{ padding: '8px', color: '#d1d5db', borderBottom: '1px solid #374151' }}>{item.productType}</td><td style={{ padding: '8px', color: '#d1d5db', borderBottom: '1px solid #374151' }}>{item.quantity}</td><td style={{ padding: '8px', color: '#d1d5db', borderBottom: '1px solid #374151' }}>${item.purchasePrice.toFixed(2)}</td><td style={{ padding: '8px', color: '#d1d5db', borderBottom: '1px solid #374151' }}>${item.currentValue.toFixed(2)}</td></tr>)}</tbody>
                    </table>
                  </div>
                  <div style={{ padding: '12px', backgroundColor: '#111827', borderRadius: '8px', marginBottom: '16px', fontSize: '12px', color: '#9ca3af' }}><p style={{ margin: 0 }}>• Duplicate products will be merged (quantities added, costs averaged)</p><p style={{ margin: '4px 0 0' }}>• Invalid rows will be skipped</p></div>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button onClick={() => setImportStep(2)} style={{ flex: 1, padding: '10px', backgroundColor: '#374151', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px' }}>Back</button>
                    <button onClick={handleImport} style={{ flex: 1, padding: '10px', backgroundColor: '#16a34a', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px' }}>Import {importData.length} Items</button>
                  </div>
                </div>
              )}
              ...
              {importStep === 4 && (
                <div style={{ textAlign: 'center', padding: '32px' }}>
                  <Icons.Loader /><p style={{ color: '#9ca3af', margin: '16px 0 8px' }}>Importing products...</p>
                  <div style={{ width: '100%', height: '8px', backgroundColor: '#374151', borderRadius: '4px', overflow: 'hidden' }}><div style={{ width: `${importProgress}%`, height: '100%', backgroundColor: '#2563eb', transition: 'width 0.3s' }}></div></div>
                  <p style={{ color: '#6b7280', fontSize: '12px', marginTop: '8px' }}>{importProgress}%</p>
                </div>
              )}
            </div>
          </div>
        ...
        {showSellForm && sellingProduct && (
          <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', zIndex: 50 }}>
            <div style={{ backgroundColor: '#1f2937', borderRadius: '8px', padding: '16px', width: '100%', maxWidth: '350px', border: '1px solid #374151' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}><h2 style={{ color: '#4ade80', fontSize: '18px', fontWeight: 'bold', margin: 0 }}>Record Sale</h2><button onClick={() => { setShowSellForm(false); setSellingProduct(null) }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af' }}><Icons.X /></button></div>
              <div style={{ padding: '12px', backgroundColor: '#374151', borderRadius: '8px', marginBottom: '12px' }}><p style={{ color: '#fb923c', fontWeight: '500', margin: 0 }}>{sellingProduct.name}</p><p style={{ color: '#9ca3af', fontSize: '12px', margin: '4px 0 0' }}>{sellingProduct.set} • {sellingProduct.quantity}x @ ${sellingProduct.purchasePrice.toFixed(2)}</p></div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        {showAddStock && addStockProduct && (
          <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', zIndex: 50 }}>
            <div style={{ backgroundColor: '#1f2937', borderRadius: '8px', padding: '16px', width: '100%', maxWidth: '350px', border: '1px solid #374151' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}><h2 style={{ color: '#60a5fa', fontSize: '18px', fontWeight: 'bold', margin: 0 }}>Add Stock</h2><button onClick={() => { setShowAddStock(false); setAddStockProduct(null) }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af' }}><Icons.X /></button></div>
              <div style={{ padding: '12px', backgroundColor: '#374151', borderRadius: '8px', marginBottom: '12px' }}><p style={{ color: '#fb923c', fontWeight: '500', margin: 0 }}>{addStockProduct.name}</p><p style={{ color: '#9ca3af', fontSize: '12px', margin: '4px 0 0' }}>Current: {addStockProduct.quantity}x @ ${addStockProduct.purchasePrice.toFixed(2)} avg</p></div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  <button onClick={handleAddStock} disabled={syncing} style={{ flex: 1, padding: '10px', backgroundColor: '#2563eb', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', opacity: syncing ? 0.5 : 1 }}>{syncing ? 'Saving...' : 'Add Stock'}</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {showSalesHistory && (
          <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', zIndex: 50 }}>
            <div style={{ backgroundColor: '#1f2937', borderRadius: '8px', padding: '16px', width: '100%', maxWidth: '700px', maxHeight: '90vh', overflow: 'auto', border: '1px solid #374151' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              ...
              {sales.length === 0 ? (
                <div style={{ padding: '40px', textAlign: 'center', color: '#6b7280' }}><p>No sales recorded yet</p></div>
              ) : (
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                    <thead><tr style={{ backgroundColor: '#111827' }}><th style={{ padding: '8px', textAlign: 'left', color: '#9ca3af', borderBottom: '1px solid #374151' }}>Date</th><th style={{ padding: '8px', textAlign: 'left', color: '#9ca3af', borderBottom: '1px solid #374151' }}>Product</th><th style={{ padding: '8px', textAlign: 'left', color: '#9ca3af', borderBottom: '1px solid #374151' }}>Qty</th><th style={{ padding: '8px', textAlign: 'left', color: '#9ca3af', borderBottom: '1px solid #374151' }}>Cost</th><th style={{ padding: '8px', textAlign: 'left', color: '#9ca3af', borderBottom: '1px solid #374151' }}>Sale</th><th style={{ padding: '8px', textAlign: 'left', color: '#9ca3af', borderBottom: '1px solid #374151' }}>P/L</th><th style={{ padding: '8px', textAlign: 'right', color: '#9ca3af', borderBottom: '1px solid #374151' }}>Actions</th></tr></thead>
                    <tbody>
                      {sales.map((s) => (
                        <tr key={s.id}>
                          <td style={{ padding: '8px', color: '#9ca3af', borderBottom: '1px solid #374151' }}>{new Date(s.date).toLocaleDateString()}</td>
                          <td style={{ padding: '8px', borderBottom: '1px solid #374151' }}><div style={{ color: '#fb923c' }}>{s.productName}</div><div style={{ color: '#6b7280', fontSize: '11px' }}>{s.set}</div></td>
                          <td style={{ padding: '8px', color: '#d1d5db', borderBottom: '1px solid #374151' }}>{s.quantity}</td>
                          <td style={{ padding: '8px', color: '#d1d5db', borderBottom: '1px solid #374151' }}>${s.purchasePrice.toFixed(2)}</td>
                          <td style={{ padding: '8px', color: '#d1d5db', borderBottom: '1px solid #374151' }}>${s.salePrice.toFixed(2)}</td>
                          <td style={{ padding: '8px', fontWeight: '500', borderBottom: '1px solid #374151', color: s.profitLoss >= 0 ? '#4ade80' : '#f87171' }}>${s.profitLoss.toFixed(2)}</td>
                          <td style={{ padding: '8px', borderBottom: '1px solid #374151', textAlign: 'right' }}>
                            {undoSaleConfirmId === s.id ? (
                              <><button onClick={() => handleUndoSale(s)} style={{ padding: '2px 8px', backgroundColor: '#2563eb', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', marginRight: '4px' }}>Y</button><button onClick={() => setUndoSaleConfirmId(null)} style={{ padding: '2px 8px', backgroundColor: '#4b5563', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px' }}>N</button></>
                            ) : deleteSaleConfirmId === s.id ? (
                              <><button onClick={() => handleDeleteSale(s.id)} style={{ padding: '2px 8px', backgroundColor: '#dc2626', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', marginRight: '4px' }}>Y</button><button onClick={() => setDeleteSaleConfirmId(null)} style={{ padding: '2px 8px', backgroundColor: '#4b5563', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px' }}>N</button></>
                            ) : (
                              <><button onClick={() => setUndoSaleConfirmId(s.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#60a5fa', padding: '4px' }} title="Undo Sale"><Icons.Undo /></button><button onClick={() => setDeleteSaleConfirmId(s.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#f87171', padding: '4px' }} title="Delete"><Icons.Trash /></button></>
                            )}
                          </td>
                        </tr>
                      ))}
              ...
              <div style={{ marginTop: '16px', padding: '12px', backgroundColor: '#111827', borderRadius: '8px', display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: '#9ca3af' }}>Total Sales: {sales.length}</span>
                <span style={{ color: realizedPL >= 0 ? '#4ade80' : '#f87171', fontWeight: 'bold' }}>Total P/L: ${realizedPL.toFixed(2)}</span>
              </div>
            </div>
          </div>
        )}
        ...
        {showDashboard && (
          <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', zIndex: 50 }}>
            <div style={{ backgroundColor: '#1f2937', borderRadius: '8px', padding: '16px', width: '100%', maxWidth: '800px', maxHeight: '90vh', overflow: 'auto', border: '1px solid #374151' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px', marginBottom: '16px' }}>
                <div style={{ backgroundColor: '#111827', padding: '16px', borderRadius: '8px' }}><p style={{ color: '#9ca3af', fontSize: '12px', margin: 0 }}>Total Investment</p><p style={{ color: '#fb923c', fontSize: '24px', fontWeight: 'bold', margin: '4px 0 0' }}>${totalCost.toFixed(2)}</p></div>
                <div style={{ backgroundColor: '#111827', padding: '16px', borderRadius: '8px' }}><p style={{ color: '#9ca3af', fontSize: '12px', margin: 0 }}>Portfolio Value</p><p style={{ color: '#4ade80', fontSize: '24px', fontWeight: 'bold', margin: '4px 0 0' }}>${totalValue.toFixed(2)}</p></div>
                <div style={{ backgroundColor: '#111827', padding: '16px', borderRadius: '8px' }}><p style={{ color: '#9ca3af', fontSize: '12px', margin: 0 }}>ROI</p><p style={{ color: unrealizedPL >= 0 ? '#4ade80' : '#f87171', fontSize: '24px', fontWeight: 'bold', margin: '4px 0 0' }}>{totalCost > 0 ? ((unrealizedPL / totalCost) * 100).toFixed(1) : 0}%</p></div>
                <div style={{ backgroundColor: '#111827', padding: '16px', borderRadius: '8px' }}><p style={{ color: '#9ca3af', fontSize: '12px', margin: 0 }}>Avg Sale Value</p><p style={{ color: '#60a5fa', fontSize: '24px', fontWeight: 'bold', margin: '4px 0 0' }}>${avgSaleValue.toFixed(2)}</p></div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '16px' }}>
                <div style={{ backgroundColor: '#111827', padding: '16px', borderRadius: '8px' }}>
                  <h3 style={{ color: '#d1d5db', fontSize: '14px', margin: '0 0 12px' }}>Inventory by Type</h3>
                  {Object.entries(typeDistribution).map(([type, count]) => (
                    <div key={type} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <div style={{ width: '100px', height: '8px', backgroundColor: '#374151', borderRadius: '4px', overflow: 'hidden' }}><div style={{ width: `${(count / totalItems) * 100}%`, height: '100%', backgroundColor: '#fb923c' }}></div></div>
                        <span style={{ color: '#d1d5db', fontSize: '12px', minWidth: '30px' }}>{count}</span>
                      </div>
                    </div>
                  ))}
                </div>
                <div style={{ backgroundColor: '#111827', padding: '16px', borderRadius: '8px' }}>
                  ...
                  {topProducts.map((p, i) => (
                    <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px', padding: '8px', backgroundColor: '#1f2937', borderRadius: '6px' }}>
                      <div><span style={{ color: '#6b7280', fontSize: '12px', marginRight: '8px' }}>#{i + 1}</span><span style={{ color: '#fb923c', fontSize: '13px' }}>{p.name}</span></div>
                      <span style={{ color: '#4ade80', fontSize: '13px', fontWeight: '500' }}>${(p.currentValue * p.quantity).toFixed(2)}</span>
                    </div>
                  ))}
                </div>
                {salesChartData.length > 0 && (
                  <div style={{ backgroundColor: '#111827', padding: '16px', borderRadius: '8px', gridColumn: 'span 2' }}>
                    <h3 style={{ color: '#d1d5db', fontSize: '14px', margin: '0 0 12px' }}>Monthly P/L</h3>
                    <div style={{ display: 'flex', alignItems: 'end', gap: '8px', height: '120px' }}>
                      {salesChartData.map((d, i) => {
                        const maxProfit = Math.max(...salesChartData.map(x => Math.abs(x.profit)))
                        const height = maxProfit > 0 ? (Math.abs(d.profit) / maxProfit) * 100 : 0
                        return (
                          <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                            <div style={{ width: '100%', height: `${height}%`, minHeight: '4px', backgroundColor: d.profit >= 0 ? '#4ade80' : '#f87171', borderRadius: '4px 4px 0 0' }}></div>
                            <span style={{ color: '#6b7280', fontSize: '10px', marginTop: '4px' }}>{d.month}</span>
                            <span style={{ color: d.profit >= 0 ? '#4ade80' : '#f87171', fontSize: '10px' }}>${d.profit}</span>
                          </div>
                        )
                      })}
                    </div>
