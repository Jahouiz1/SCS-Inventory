...
// Admin credentials - set these in your .env.local file
// NEXT_PUBLIC_ADMIN_PASSWORD=your_secure_password
const ADMIN_PASSWORD = process.env.NEXT_PUBLIC_ADMIN_PASSWORD || 'scs2024'
...
const Icons = {
  ...
  Lock: () => <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg>,
...
export default function SecretCardSociety() {
  ...
  useEffect(() => {
    const session = localStorage.getItem('scs_admin_session')
    if (session) {
      ...
      if (Date.now() - timestamp < sevenDays) {
        ...
        localStorage.removeItem('scs_admin_session')
      }
    }
  ...
  const handleLogin = (e) => {
    ...
    if (loginPassword === ADMIN_PASSWORD) {
      ...
      setLoginError('')
      setLoginPassword('')
      localStorage.setItem('scs_admin_session', JSON.stringify({ timestamp: Date.now() }))
    } else {
  ...
  if (authLoading) return (
    <div style={{ minHeight: '100vh', backgroundColor: '#111827', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <Icons.Loader />
      <p style={{ color: '#9ca3af', marginLeft: '8px' }}>Loading...</p>
    </div>
  )
  ...
  if (!isAuthenticated) return (
    <div style={{ minHeight: '100vh', backgroundColor: '#111827', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
      <div style={{ backgroundColor: '#1f2937', borderRadius: '16px', padding: '32px', width: '100%', maxWidth: '400px', border: '1px solid #374151', textAlign: 'center' }}>
        <div style={{ color: '#fb923c', marginBottom: '16px' }}>
          <Icons.Lock />
        </div>
        <h1 style={{ color: '#fb923c', fontSize: '24px', fontWeight: 'bold', marginBottom: '8px' }}>🔐 Secret Card Society</h1>
        <p style={{ color: '#9ca3af', fontSize: '14px', marginBottom: '24px' }}>Admin access required</p>
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '8px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <button onClick={handleLogout} style={{ padding: '6px 12px', backgroundColor: '#dc2626', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '12px', display: 'flex', alignItems: 'center', gap: '4px' }} title="Logout"><Icons.Logout /> Logout</button>
          </div>
        </div>
        ...
        {showSalesHistory && (
          <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', zIndex: 50 }}>
            <div style={{ backgroundColor: '#1f2937', borderRadius: '8px', padding: '16px', width: '100%', maxWidth: '600px', maxHeight: '90vh', overflow: 'auto', border: '1px solid #374151' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <h2 style={{ color: '#d1d5db', fontSize: '18px', fontWeight: 'bold', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}><Icons.History /> Sales History</h2>
                <button onClick={() => setShowSalesHistory(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af' }}><Icons.X /></button>
              </div>
              {sales.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '32px', color: '#6b7280' }}>No sales recorded yet</div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <div style={{ padding: '12px', backgroundColor: '#111827', borderRadius: '8px', marginBottom: '8px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ color: '#9ca3af', fontSize: '13px' }}>Total Sales: {sales.length}</span>
                      <span style={{ color: sales.reduce((sum, s) => sum + s.profitLoss, 0) >= 0 ? '#4ade80' : '#f87171', fontSize: '14px', fontWeight: 'bold' }}>
                        P/L: {sales.reduce((sum, s) => sum + s.profitLoss, 0) >= 0 ? '+' : ''}${sales.reduce((sum, s) => sum + s.profitLoss, 0).toFixed(2)}
                      </span>
                    </div>
                  </div>
                  {sales.map(sale => (
                    <div key={sale.id} style={{ padding: '12px', backgroundColor: '#111827', borderRadius: '8px', border: '1px solid #374151' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                        <div>
                          <div style={{ color: '#fb923c', fontWeight: 'bold', fontSize: '14px' }}>{sale.productName}</div>
                          <div style={{ color: '#9ca3af', fontSize: '11px' }}>{sale.set} • {sale.condition}{sale.grade && ` • ${sale.grade}`}</div>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ color: sale.profitLoss >= 0 ? '#4ade80' : '#f87171', fontWeight: 'bold', fontSize: '14px' }}>
                            {sale.profitLoss >= 0 ? '+' : ''}${sale.profitLoss.toFixed(2)}
                          </div>
                          <div style={{ color: '#6b7280', fontSize: '10px' }}>{new Date(sale.date).toLocaleDateString()}</div>
                        </div>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '12px', color: '#6b7280' }}>
                        <span>Qty: {sale.quantity} × ${sale.salePrice.toFixed(2)}</span>
                        <span>Cost: ${sale.purchasePrice.toFixed(2)}</span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', marginTop: '8px' }}>
                        {undoSaleConfirmId === sale.id ? (
                          <>
                            <span style={{ color: '#fbbf24', fontSize: '11px', alignSelf: 'center' }}>Undo sale?</span>
                            <button onClick={() => handleUndoSale(sale)} disabled={syncing} style={{ padding: '4px 8px', backgroundColor: '#ca8a04', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px' }}>Yes</button>
                            <button onClick={() => setUndoSaleConfirmId(null)} style={{ padding: '4px 8px', backgroundColor: '#374151', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px' }}>No</button>
                          </>
                        ) : deleteSaleConfirmId === sale.id ? (
                          <>
                            <span style={{ color: '#f87171', fontSize: '11px', alignSelf: 'center' }}>Delete?</span>
                            <button onClick={() => handleDeleteSale(sale.id)} disabled={syncing} style={{ padding: '4px 8px', backgroundColor: '#dc2626', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px' }}>Yes</button>
                            <button onClick={() => setDeleteSaleConfirmId(null)} style={{ padding: '4px 8px', backgroundColor: '#374151', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px' }}>No</button>
                          </>
                        ) : (
                          <>
                            <button onClick={() => setUndoSaleConfirmId(sale.id)} style={{ padding: '4px 8px', backgroundColor: '#374151', color: '#fbbf24', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', display: 'flex', alignItems: 'center', gap: '4px' }}><Icons.Undo /> Undo</button>
                            <button onClick={() => setDeleteSaleConfirmId(sale.id)} style={{ padding: '4px 8px', backgroundColor: '#374151', color: '#f87171', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', display: 'flex', alignItems: 'center', gap: '4px' }}><Icons.Trash /> Delete</button>
                          </>
                        )}
                      </div>
                    </div>
